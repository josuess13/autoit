...
Func Venda_balanca($cliente = 0, $cod_balanca = 0, $total_esperado_venda = "", $produto = "")
    ...
    ControlSetText($tela_venda_dav, "", "TMyEdit15", $cod_balanca)
	...
	If $valor_total_venda <> $total_esperado_venda Then
		_ScreenCapture_Capture($config &"-"& $cliente & "-" & $produto & " (Erro_valor_Total_diferente_esperado).png")
    EndIf
    IniWrite("ClienteBalanca"& $config & ".txt", $produto & ". Cliente " & $cliente, "Esperado:" & $total_esperado_venda & " ", " Resultado:" & $valor_total_venda)

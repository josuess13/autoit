...
; Inserindo cliente
	...
	EndIf
; Inserindo Produto
	ControlClick($tela_venda, "", "TMyEdit17", "left", 1)
	ControlSend($tela_venda, "", "TMyEdit17", $produto)
	Send("{enter}")
	ControlClick($tela_venda, "", "TMyEdit16", "left", 1)
	ControlSend($tela_venda, "", "TMyEdit16", $cod_prod)
	Send("{enter}")
	ControlClick($tela_venda, "", "TMyEdit15", "left", 1)
	Send("{enter}")
	ControlClick($tela_venda, "", "TMyEdit11", "left", 1)
	Send("{enter}")
	Sleep(1000)
; Capturando preço produto
	$preco_produto = ControlGetText($tela_venda, "","TMyEdit11")
...
; Inserindo Produto 34700
	...
	Sleep(1000)
; Capturando preço produto
	$valor_carregado_item = ControlGetText($tela_venda, "","TMyEdit11")

...
Func venda($cliente =0, $produto = 0, $cod_prod = 0, $total_esperado_venda = "", $tipo_venda = "", $alterar_preco = "")
...
; Capturando valor total da venda
	...
	If $valor_total_venda <> $total_esperado_venda Then
		_ScreenCapture_Capture($tipo_venda & $produto & "(Erro_valor_Total_diferente_esperado).png")
	EndIf
; Salvando arquivo de texto com os valores

	IniWrite("Kit.txt", $tipo_venda, "Produto " & $produto & "- Esperado total", $total_esperado_venda)
	IniWrite("Kit.txt", $tipo_venda, "Produto " & $produto & "- Inserido total", $valor_total_venda)
